//-----------------------------------------------------------------
// Game Engine Object
// C++ Header - GameEngine.h - version v2_12 jan 2013 
...
struct DOUBLE2
{
	...
	DOUBLE2&		operator+=(DOUBLE2 other);						// Translate this point inplace:						p += v;
    DOUBLE2&		operator-=(DOUBLE2 other);						// Translate this point inplace (opposite direction):	p -= v;
	DOUBLE2&		operator*=(double factor);						// Scale vector inplace:								v *= s;
	DOUBLE2&		operator/=(double divisor);						// Downscale vector inplace:							v /= s;
...
public:
	...
	bool			SetGameValues(String title, WORD wIcon, WORD wSmallIcon, int iWidth, int iHeight);
	bool			GoFullscreen();		
	bool			GoWindowedMode();		
	bool			IsFullscreen();		
	...
	bool			DrawPolygon(DOUBLE2 ptsArr[], int count, bool close, double strokeWidth = 1.0);
	bool			DrawPolygon(POINT ptsArr[], int count, bool close);
	bool			FillPolygon(DOUBLE2 ptsArr[], int count);	
	bool			FillPolygon(POINT ptsArr[], int count);
	...
	bool			DrawEllipse(DOUBLE2 centerPt, int radiusX, int radiusY, double strokeWidth = 1.0);
	bool			DrawEllipse(int centerX, int centerY, int radiusX, int radiusY);
	bool			FillEllipse(DOUBLE2 centerPt, int radiusX, int radiusY);
	bool			FillEllipse(int centerX, int centerY, int radiusX, int radiusY);
	bool			DrawString(String text, DOUBLE2 topLeft, double right = -1, double bottom = -1);
	bool			DrawString(String text, int xPos, int yPos, int right = -1, int bottom = -1);
	...
	// Mutator Methods	
	void SetTitle(String title); // SetTitle automatically sets the window class name to the same name as the title - easier for students 
	void SetIcon(WORD wIcon);
	void SetSmallIcon(WORD wSmallIcon);
	void SetWidth(int iWidth);
	void SetHeight(int iHeight);
	void SetSleep(bool bSleep);
	void SetKeyList(String keyList);
...
public:
	...
	void SetFont(String fontName, bool bold, bool italic, bool underline, int size);
...
private:	
	...
	void SendMCICommand(String command);
...
public:
	...
	// Constructors & Destructor
	// -------------------------
	Bitmap(String fileName);							// Load an Bitmap using a filename
...
public:
	...
	Font(IDWriteTextFormat *textFormatPtr);
...
private:
	...
	// assignment operator.
	// -------------------------
	Font(const Font& yRef);									
	Font& operator=(const Font& yRef);	
};
...
public:
    ...
    // All points on the ray are p + lambda*v, where lambda >= 0
    // Returns all hits with lambda >= 0 && lambda < maxLambda.
    // Return number of hits.
    // Ignores hits facing in the same direction as v,
    // i.e. with dot(normal, v) * sense > 0
    int Raycast(DOUBLE2 p, DOUBLE2 v, double maxLambda, HIT* hitArr, int hitArrSize, double sense=-1);
...
public: 
	...
	// General Methods
	//---------------------------
	bool CreateFromRect(double left, double top, double right, double bottom);						// Creates a rectangular HitRegion in object space
	...
	bool CreateFromSVG(String filePath);														// Load geometry from a file: Currently only a tiny subset of SVG is supported, namely groups and paths (but no arcs)
