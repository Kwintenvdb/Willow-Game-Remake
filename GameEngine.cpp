//-----------------------------------------------------------------
// Game Engine Object
// C++ Source - GameEngine.cpp - version v2_12 jan 2013 
...
GameEngine::~GameEngine()
{
	...
	//DiscardDeviceResources();//causes crash
...
//-----------------------------------------------------------------
void GameEngine::SetTitle(String title)
{
	delete m_TitlePtr; // delete the title string if it already exists
	m_TitlePtr = new String(title);
}
	...
	while (true)
	{
		...
		else
		{
			...
			if (m_bSleep == false && m_bRunGameLoop == true)
			{
				...
				//double tickTimeNow = m_GameTickTimerPtr->GetDeltaTime();
				double tickTimeNow = m_GameTickTimerPtr->GetGameTime();
				m_GameTickTimerPtr->Reset();
				m_GamePtr->CheckKeyboard();
				m_GamePtr->GameTick(tickTimeNow);
 				ExecuteDirect2DPaint();//this waits for vertical blanking interval (avoids "tearing")
			}
...
bool GameEngine::SetGameValues(String title, WORD wIcon, WORD wSmallIcon, int iWidth = 640, int iHeight = 480)
{
	SetTitle(title);
	SetIcon(wIcon);
...
void GameEngine::SetKeyList(String keyList)
{
	if (keyList.ToTChar() != NULL) delete m_KeyListPtr;				// clear the list if one already exists

	int iLength = 0;

	while (keyList.ToTChar()[iLength] != '\0') iLength++;			// count the amount of characters
	...
	{
		TCHAR key = keyList.ToTChar()[count]; 
...
void GameEngine::MessageBox(String text)
{
	if(m_TitlePtr==nullptr)m_TitlePtr=new String("No Title");
	if (sizeof(TCHAR) == 2)	MessageBoxW(GetWindow(), (wchar_t*) text.ToTChar(), (wchar_t*) m_TitlePtr->ToTChar(), MB_ICONEXCLAMATION | MB_OK);
	else MessageBoxA(GetWindow(), (char*) text.ToTChar(), (char*) m_TitlePtr->ToTChar(), MB_ICONEXCLAMATION | MB_OK);
}
...
{
	if (m_IsDoublebuffering) return true;
	else
	{
...
bool GameEngine::DrawPolygon(POINT ptsArr[], int count, bool close)
{
	if (!CanIPaint()) return false;
	...
	for (int countLoop = 0; countLoop < count - 1; ++countLoop)
	{
...
bool GameEngine::DrawPolygon(DOUBLE2 ptsArr[], int count, bool close, double strokeWidth)
{
	if (!CanIPaint()) return false;
...
bool GameEngine::FillPolygon(POINT ptsArr[], int count)
{
	if(!CanIPaint()) return false;
...
bool GameEngine::FillPolygon(DOUBLE2 ptsArr[], int count)
{
	if(!CanIPaint())return false;
...
bool GameEngine::DrawEllipse(DOUBLE2 centerPt, int radiusX, int radiusY, double strokeWidth)
{
	if (!CanIPaint()) return false;
...
bool GameEngine::FillEllipse(DOUBLE2 centerPt, int radiusX, int radiusY)
{
	if (!CanIPaint()) return false;
...
bool GameEngine::DrawString(String text, DOUBLE2 topLeft, double right, double bottom)
{
	...
	if (right == -1) right = (int) (dstSize_f.width);
	if (bottom == -1) bottom = (int) (dstSize_f.height);
	...
	m_RenderTargetPtr->DrawText(wText.c_str(), text.GetLength(), m_UserFontPtr->GetTextFormat(), layoutRect, m_ColorBrushPtr, D2D1_DRAW_TEXT_OPTIONS_CLIP);
...
LRESULT GameEngine::HandleEvent(HWND hWindow, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
		...
		//m_GamePtr->MouseButtonAction(true, true, (int) LOWORD(lParam), (int) HIWORD(lParam));
		return 0;

	case WM_LBUTTONUP:
		m_GamePtr->MouseButtonAction(true, false, (int) LOWORD(lParam), (int) HIWORD(lParam), wParam);
		//m_GamePtr->MouseButtonAction(true, false, (int) LOWORD(lParam), (int) HIWORD(lParam));
		return 0;

	case WM_RBUTTONDOWN:
		m_GamePtr->MouseButtonAction(false, true, (int) LOWORD(lParam), (int) HIWORD(lParam), wParam);
		//m_GamePtr->MouseButtonAction(false, true, (int) LOWORD(lParam), (int) HIWORD(lParam));
		return 0;

	case WM_RBUTTONUP:
		m_GamePtr->MouseButtonAction(false, false, (int) LOWORD(lParam), (int) HIWORD(lParam), wParam);
		//m_GamePtr->MouseButtonAction(false, false, (int) LOWORD(lParam), (int) HIWORD(lParam));
		return 0;

	case WM_MOUSEMOVE:
		m_GamePtr->MouseMove((int) LOWORD(lParam), (int) HIWORD(lParam), wParam);
		//m_GamePtr->MouseMove((int) LOWORD(lParam), (int) HIWORD(lParam));
		return 0;
...
Audio::Audio(String name) : m_Playing(false), m_Paused(false), m_MustRepeat(false), m_hWnd(0), m_Volume(100)
{	
...
Audio::Audio(int IDAudio, String type) : m_Playing(false), m_Paused(false), m_MustRepeat(false), m_hWnd(0), m_Volume(100)
{
	if (type == "MP3" || type == "WAV" || type == "MID")
	{
		m_Alias = String("audio") + m_Nr++;
		m_FileName = String("temp\\") + m_Alias;

		if (type == "MP3") m_FileName += ".mp3";
		else if (type == "WAV") m_FileName += ".wav";
		else m_FileName += ".mid";

		Extract(IDAudio, type, m_FileName);

		Create(m_FileName);
	}
}

void Audio::Create(String name)
{
	TCHAR buffer[100];

	String sendString;

	if (name.EndsWith(".mp3")) sendString = String("open \"") + m_FileName + "\" type mpegvideo alias " + m_Alias;
	else if (name.EndsWith(".wav")) sendString = String("open \"") + m_FileName + "\" type waveaudio alias " + m_Alias;
	else if (name.EndsWith(".mid")) sendString = String("open \"") + m_FileName + "\" type sequencer alias " + m_Alias;
...
void Audio::QueueCommand(String command)
{
...
void Audio::SendMCICommand(String command)
{
	int result = mciSendString(command.ToTChar(), 0, 0, m_hWnd);
}
...
#pragma warning(disable:4311)	
#pragma warning(disable:4312)
TextBox::TextBox(String text) : m_X(0), m_Y(0), m_BgColor(RGB(255, 255, 255)), m_ForeColor(RGB(0, 0, 0)), m_BgColorBrush(0), m_Font(0), m_OldFont(0)
{
	// Create the edit box
	m_hWndEdit = CreateWindow(TEXT("EDIT"), text.ToTChar(), WS_BORDER | WS_CHILD | WS_CLIPSIBLINGS | WS_TABSTOP | ES_LEFT | ES_AUTOHSCROLL, 0, 0, 0, 0, GameEngine::GetSingleton()->GetWindow(), NULL, GameEngine::GetSingleton()->GetInstance(), NULL);
...
{
	SendMessage(m_hWndEdit, WM_SETTEXT, 0, (LPARAM) text.ToTChar());
}

void TextBox::SetFont(String fontName, bool bold, bool italic, bool underline, int size)
{
	LOGFONT ft;

	_tcscpy_s(ft.lfFaceName, sizeof(ft.lfFaceName) / sizeof(TCHAR), fontName.ToTChar());
	ft.lfStrikeOut = 0;
...
COLOR TextBox::GetForecolor()
{
	return COLOR(GetRValue(m_ForeColor),GetGValue(m_ForeColor),GetBValue(m_ForeColor));
}

COLOR TextBox::GetBackcolor()
{
	return COLOR(GetRValue(m_BgColor),GetGValue(m_BgColor),GetBValue(m_BgColor));
}
...
LRESULT TextBox::EditProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	{		
	case WM_CTLCOLOREDIT:
		SetBkColor((HDC) wParam, m_BgColor);
...
#pragma warning(disable:4311)
#pragma warning(disable:4312)
Button::Button(String text) : m_X(0), m_Y(0), m_Armed(false), m_Font(0), m_OldFont(0)
{
	// Create the button object
	m_hWndButton = CreateWindow(TEXT("BUTTON"), text.ToTChar(), WS_BORDER | WS_CHILD | WS_CLIPSIBLINGS | WS_TABSTOP | BS_PUSHBUTTON, 0, 0, 0, 0, GameEngine::GetSingleton()->GetWindow(), NULL, GameEngine::GetSingleton()->GetInstance(), NULL);
...
{
	SendMessage(m_hWndButton, WM_SETTEXT, 0, (LPARAM) text.ToTChar());
}

void Button::SetFont(String fontName, bool bold, bool italic, bool underline, int size)
{
	LOGFONT ft;
...
//-----------------------------------------------------------------
void OutputDebugString(String text)
{
	OutputDebugString(text.ToTChar());
}
...
void GameEngine::CreateDeviceResources()
{
	...
	if (!m_RenderTargetPtr)
	{
		...
		//Create a Font
		m_DefaultFontPtr = new Font(_T("Consolas"), 12);
		m_UserFontPtr = m_DefaultFontPtr;
		m_bInitialized = true;
	}
}
...
Bitmap::Bitmap(String fileName) : m_BitmapPtr(nullptr), m_ConvertorPtr(nullptr), m_Opacity(1.0), m_ResourceID(0)
{
...
Font::Font(String fontName, float size)
{
...
{
	return hit1.lambda < hit2.lambda;
}
...
int HitOutline::Raycast(DOUBLE2 p, DOUBLE2 v, double maxLambda, HIT* hitArr, int hitArrSize, double sense)
{
	//local data: Bart
	...
	if( lineVertexCount > 0 )
	{
		...
		for(int count = 0; count < lineVertexCount && hitCount < MAX_HITS; count += 2 )
		{
			...
			//create two AABB, and check if there is an opening between them
			//reduces the raycast time for large amounts of lines
			RECT2 r1(min(p1.x,p2.x),min(p1.y,p2.y),max(p1.x,p2.x),max(p1.y,p2.y));
			RECT2 r2(min(q1.x,q2.x),min(q1.y,q2.y),max(q1.x,q2.x),max(q1.y,q2.y));
			bool noIntersect= r1.right < r2.left || r1.left> r2.right || r1.top> r2.bottom || r1.bottom < r2.top;
			if(!noIntersect)
			{
				...
				if( (ortho.DotProduct(v) * sense < 0) || sense == 0)
				{
					...
					if(IntersectLines(p1, p2, q1, q2, lambda1, lambda2))
					{
						if( lambda1 >= 0 && lambda1 < maxLambda && lambda2 >= 0 && lambda2 <= 1 )
						{
							HIT hit;
							hit.lambda = lambda1;
							hit.point = p + lambda1 * v;
	...
	hitCount = min(hitCount, hitArrSize);
	for(int count = 0; count < hitCount; ++count )
	{
...
// http://www.digitalartsandentertainment.be/
//-----------------------------------------------------------------
HitRegion::HitRegion() : m_D2DFactoryPtr(GameEngine::GetSingleton()->GetD2DFactory()), m_GeometryPtr(nullptr), m_TransformedGeometryPtr(nullptr), m_HitOutlinePtr(nullptr)
{}

HitRegion::HitRegion(ID2D1Factory* d2DFactoryPtr) : m_D2DFactoryPtr(d2DFactoryPtr), m_GeometryPtr(nullptr), m_TransformedGeometryPtr(nullptr)
{}

HitRegion::~HitRegion()
{
	if (m_GeometryPtr != nullptr) m_GeometryPtr->Release();
	if (m_TransformedGeometryPtr != nullptr) m_TransformedGeometryPtr->Release();
	if (m_HitOutlinePtr != nullptr) delete m_HitOutlinePtr;
}

bool HitRegion::CreateFromRect(double left, double top, double right, double bottom)
{
	ID2D1RectangleGeometry* rgPtr = nullptr;
	m_HResult = m_D2DFactoryPtr->CreateRectangleGeometry(D2D1::RectF((FLOAT) left, (FLOAT) top, (FLOAT) (right), (FLOAT) (bottom)), &rgPtr);
...
{
	ID2D1GeometrySink* sinkPtr = nullptr;
	...
	if (SUCCEEDED(hr))
	{
		...
		if (sinkPtr != nullptr) sinkPtr->Release();
	}
	if (SUCCEEDED(hr))
	{
		if (m_GeometryPtr != nullptr) m_GeometryPtr->Release();
...
bool HitRegion::CreateFromSVG(String filePath)
{
	SVGLoader sVGLoader;
...
{
	return m_GeometryPtr==nullptr?false:true;
}	
...
RECT2 HitRegion::CollisionTest(HitRegion* regPtr)
{
	...
	ID2D1PathGeometry* pathGeometryIntersectPtr = nullptr;
	ID2D1GeometrySink* geometrySinkPtr = nullptr;
	hr = m_D2DFactoryPtr->CreatePathGeometry(&pathGeometryIntersectPtr);
	hr = pathGeometryIntersectPtr->Open(&geometrySinkPtr);
	if (SUCCEEDED(hr))
	{
		hr = m_TransformedGeometryPtr->CombineWithGeometry(regPtr->GetTransformedGeometry(), D2D1_COMBINE_MODE_INTERSECT, NULL, NULL, geometrySinkPtr);
	}
	else
	{
		DebugBreak();
	...
	if (SUCCEEDED(hr))
	{
		hr = pathGeometryIntersectPtr->GetBounds(D2D1::IdentityMatrix(), &bounds);
	...
	pathGeometryIntersectPtr->Release();
...
int HitRegion::Raycast(DOUBLE2 startPoint, DOUBLE2 vector, HIT* hitArr, int hitArrSize, int sense)
{
	...
	{
		hitArr[count].point = m_MatTransform.TransformPoint(hitArr[count].point);
		hitArr[count].normal = m_MatTransform.TransformVector(hitArr[count].normal);
	}
	return numHits;
}
...
void HitRegion::SetPos(double x, double y)
{
	...
	MATRIX3X2 matTranslate = MATRIX3X2::CreateTranslationMatrix((FLOAT) x, (FLOAT) y);
	SetTransformMatrix(matTranslate);
}
...
HitRegion* HitRegion::Combine(HitRegion* meshPtr, CombineMode mode)
{
	...
	if (SUCCEEDED(hr))
	{
		hr = m_GeometryPtr->CombineWithGeometry(meshPtr->GetTransformedGeometry(), combineMode, NULL, NULL, geometrySinkPtr);
	}
	else
	{
		DebugBreak();
	...
	if (SUCCEEDED(hr))
	{
		...
		hitRegionPtr->m_GeometryPtr = pathGeometryIntersectPtr;
		hitRegionPtr->CreateTransformedGeometry();
		return hitRegionPtr;
	}
	else
	{
...
void HitRegion::CreateTransformedGeometry()
{
...
{
	HitOutline* newHitOutlinePtr = nullptr;
	ID2D1GeometrySink * sinkPtr = nullptr;
	ID2D1PathGeometry * pathGeometryPtr;
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			newHitOutlinePtr = new HitOutline(m_GeometryPtr, sinkPtr, maxDegree, flatteningTolerance);
			hr = sinkPtr->Close();
		}
		sinkPtr->Release();
	}
